...
### Installation manuelle
  ...
  Allez sur le dépôt GitHub du plugin et cliquez sur le lien "Download ZIP" qui est proposé (en bas à droite) et décompressez l'archive obtenue dans votre répertoire `plugins`.

  > Important : par défaut, le zip créé par github et le dossier contenu dans l'archive sont de la forme `docalist-search-master`. Quand vous décompressez l'archive vous **devez** renommer le répertoire et enlever le suffixe `-master`. Par exemple pour docalist-search, le répertoire d'installation dans votre WordPress doit être `plugins/docalist-search` et non pas `plugins/docalist-search-master`.
...
### Installation via GitHub Updater
  ...
  Tous les plugins docalist sont compatibles avec GitHub Updater. Pour installer les plugins docalist, il suffit d'aller dans la page "réglages" de GitHub Updater et de coller l'url du dépôt du plugin docalist (cf. plus haut).
...
### Installation via Git
  ...
  > Remarque : GitHub affiche les adresses à utiliser en bas à droite de chaque dépôt
...
### Installation via Composer
  ...
  **Remarque :** quand les plugins docalist seront dispos sur [Packagist](https://packagist.org/) (et sur [WPackagist](http://wpackagist.org/)), ce sera encore plus simple, car il suffira de requérir un plugin pour installer automatiquement ses dépendances :

  ```sh
  $ composer require docalist/docalist-search-book-demo
  ```
  (installera aussi docalist-search, docalist-core, etc.)
...
> Les plugins docalist disposent d'un système de détection des dépendances qui fait que vous ne pourrez pas activer un plugin si les autres plugins dont il a besoin ne sont pas activés.
>
...
Sur les systèmes qui utilisent un gestionnaires de paquets (apt, yum, rpm...), c'est encore plus simple car il suffit de lancer une commande pour faire l'installation ou la mise à jour (par exemple, sur une debian, il suffit de faire `sudo apt-get install elasticsearch` après avoir mis à jour le fichier `sources.list`).
...
1. Vérifiez la version de java dont vous disposez : ElasticSearch requiert java 7 (et de préférence java 8).
  ...
  >
  > Sous Windows : panneau de configuration » java » onglet "Général", bouton "à propos"
  >
...
2. Vérifiez que la variable d'environnement `JAVA_HOME` est correctement définie
  ...
  >
  > Si ce n'est pas le cas, repérez dans quel répertoire est installé java et utilisez une commande du style `export JAVA_HOME=répertoire`.
  >
...
5. Lancez l'exécutable ElasticSearch. Il se trouve dans `esdir/bin/elasticsearch` (`.bat` si vous êtes sous Windows).

  > Si vous lancez l'application en ligne de commande, cela vous permettra de voir ce qui se passe (affichage des logs au fur et à mesure) et d'arrêter facilement l'application (`ctrl+break` !).
...
De plus en plus de sociétés proposent de services Elastic Search en mode SaaS (hébergé). On peut citer par exemple :
...
> A noter : à ma connaissance, [bonsai.io](http://bonsai.io/) est l'une des seules sociétés à proposer une offre découverte, gratuite et non limitée dans le temps : localisation en Irlande, 10 000 documents au plus, 1Go de disque et de RAM, aucune redondance.
>
